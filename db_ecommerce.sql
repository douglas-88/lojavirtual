...
--
-- Host: localhost:3306
-- Tempo de geração: 28/12/2019 às 20:35
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_products_save` (IN `pidproduct` INT(11), IN `pdesproduct` VARCHAR(100), IN `pvlprice` DECIMAL(10,2), IN `pvlwidth` DECIMAL(10,2), IN `pvlheight` DECIMAL(10,2), IN `pvllength` DECIMAL(10,2), IN `pvlweight` DECIMAL(10,2), IN `pdesurl` VARCHAR(128), IN `ppathphoto` VARCHAR(400))  BEGIN
	...
	IF pidproduct > 0 THEN
        ...
        SET 
            ...
            vlweight = pvlweight,
            desurl = pdesurl,
            pathphoto = ppathphoto
        WHERE idproduct = pidproduct;
        
    ELSE
		
		INSERT INTO tb_products (desproduct, vlprice, vlwidth, vlheight, vllength, vlweight, desurl,pathphoto) 
        VALUES(pdesproduct, pvlprice, pvlwidth, pvlheight, pvllength, pvlweight, pdesurl,ppathphoto);
...
CREATE TABLE `tb_products` (
  `idproduct` int(11) NOT NULL,
  `desproduct` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  ...
  `pathphoto` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL,
  `dtregister` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
ALTER TABLE `tb_cartsproducts`
  MODIFY `idcartproduct` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=24;
--
...
--
ALTER TABLE `tb_persons`
  MODIFY `idperson` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=30;
--
...
--
ALTER TABLE `tb_addresses`
  ADD CONSTRAINT `fk_addresses_persons` FOREIGN KEY (`idperson`) REFERENCES `tb_persons` (`idperson`);

--
-- Restrições para tabelas `tb_carts`
--
ALTER TABLE `tb_carts`
  ADD CONSTRAINT `fk_carts_users` FOREIGN KEY (`iduser`) REFERENCES `tb_users` (`iduser`);

--
-- Restrições para tabelas `tb_cartsproducts`
--
ALTER TABLE `tb_cartsproducts`
  ADD CONSTRAINT `fk_cartsproducts_carts` FOREIGN KEY (`idcart`) REFERENCES `tb_carts` (`idcart`),
  ADD CONSTRAINT `fk_cartsproducts_products` FOREIGN KEY (`idproduct`) REFERENCES `tb_products` (`idproduct`);
